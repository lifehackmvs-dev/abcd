...
**Этот проект представляет собой Telegram-бота для продажи VPN-подписок, интегрированного с KeystoneWave.**
...
> [!WARNING]
> **Последняя версия бота совместима только с панелью KeystoneWave версии 2.3–2.4.x**  
...
- **`APP_DOMAIN`** : Домен, используемый Telegram и KeystoneWave для доступа к вашему боту.
...
- **`KEYSTONEWAVE_HOST`** : Домен или имя контейнера Docker.
- **`KEYSTONEWAVE_TOKEN`** : API-токен KeystoneWave, созданный в панели.
- **`KEYSTONEWAVE_WEBHOOK_SECRET`** : Должен совпадать со значением `WEBHOOK_SECRET_HEADER` из `.env` панели.

> [!WARNING]
> Также в зависимости от вашей конфигурации обратите внимание на данные переменные: **`BOT_MINI_APP`**, **`KEYSTONEWAVE_CADDY_TOKEN`**, **`KEYSTONEWAVE_COOKIE`**.

> [!IMPORTANT]
> Для корректной работы бота необходимо правильно настроить вебхук.  
> В файле `.env` панели KeystoneWave укажите:
> ```
...
Для корректной работы Keystone Tech требуется настроенный обратный прокси.  
Он необходим для получения вебхуков от Telegram, панели KeystoneWave и платежных систем.

**В этом руководстве не описывается установка или настройка прокси.**  
Вы можете использовать любой прокси-сервер по аналогии с [**KeystoneWave**](https://docs.keystonewave.com/docs/install/reverse-proxies/).
