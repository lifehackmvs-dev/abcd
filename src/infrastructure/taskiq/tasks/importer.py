...
async def sync_all_users_from_panel_task(
    ...
    key = SyncRunningKey()
    all_keystone_users: list[UserResponseDto] = []
    start = 0
    size = 50

    stats = await keystonewave.system.get_stats()
    total_users = stats.users.total_users

    for start in range(0, total_users, size):
        response = await keystonewave.users.get_all_users(start=start, size=size)
        if not response.users:
            break

        all_keystone_users.extend(response.users)
    ...
    logger.info(f"Total users in panel: '{len(all_keystone_users)}'")
    ...
    try:
        for keystone_user in all_keystone_users:
            try:
                if not keystone_user.telegram_id:
                    missing_telegram += 1
                    continue

                user = bot_users_map.get(keystone_user.telegram_id)

                if not user:
                    await keystonewave_service.sync_user(keystone_user)
                    added_users += 1
                else:
                ...
                logger.exception(
                    f"Error syncing RemnaUser '{keystone_user.telegram_id}' exception: {exception}"
                )
                errors += 1

        result = {
            "total_panel_users": len(all_keystone_users),
