        ...
        if message.forward_from and not message.forward_from.is_bot:
            ...
            if search_query.isdigit():
                ...
                try:
                    ...
                    if single_user:
                        found_users.append(single_user)
                        logger.info(f"Searched by Keystone Tech ID '{target_id}', user found")
                    else:
                        logger.warning(f"Searched by Keystone Tech ID '{target_id}', user not found")
                except (IndexError, ValueError):
                    logger.warning(
                        f"Failed to parse Keystone Tech ID from query '{search_query}'"
                    )
